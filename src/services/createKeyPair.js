...
const describeKeyPairsParams = {
  KeyNames: [createKeyPairParams.KeyName],
  DryRun: false,
};
...
const createKeyPair = async () => {
  ...
  ec2.describeKeyPairs(describeKeyPairsParams, (error) => {
    if (!error) {
      console.log(`"${describeKeyPairsParams.KeyNames}" already exists. Skipping...`);
    } else {
      console.log(`"${describeKeyPairsParams.KeyNames}" do not exist! Creating one...`);
      ec2.createKeyPair(createKeyPairParams, (err, newKeyPair) => {
        if (err) throw err;
